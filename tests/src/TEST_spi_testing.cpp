...
TEST_CASE("bus_testing, Empty SPI test bus has empty buffers") {
    spi::bus_testing test_empty_buffer;
    REQUIRE(test_empty_buffer.in_buffer_index == 0);
    REQUIRE(test_empty_buffer.out_buffer_size == 0);
}

TEST_CASE("bus_testing, SPI test bus can test write operations") {
    spi::bus_testing test_write;
...
TEST_CASE("bus_testing, SPI test bus can test singe byte write operations") {
    spi::bus_testing test_byte_write;
...
TEST_CASE("bus_testing, SPI test bus can test read operations") {
    spi::bus_testing test_read;
...
TEST_CASE("bus_testing, SPI test bus can test single byte read operations") {
    spi::bus_testing test_byte_read;
...
TEST_CASE("bus_testing, SPI test bus read can be cleared") {
    spi::bus_testing test_clear_read;
    std::array<uint8_t, 3> data = {18, 16, 14};
...
TEST_CASE("bus_testing, SPI test bus write can be cleared") {
    spi::bus_testing test_clear_write;
