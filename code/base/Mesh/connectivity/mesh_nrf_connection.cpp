...
namespace mesh {
    void mesh_nrf_connection::flush(nrf24l01plus &nrf) {
        switch (connection_state) {
            ...
            case mesh::RESPONDED:
            case mesh::ACCEPTED:
                nrf.rx_set_address(pipe_number, nrfaddress);
                nrf.rx_enabled(pipe_number, true);
                if (pipe_number == 0) {
                    nrf.tx_set_address(nrfaddress);
                }
                break;
        }
    }
    ...
    bool mesh_nrf_connection::send_message(array<mesh_nrf_connection, 6> &all_pipes, nrf24l01plus &nrf,
        ...
        nrf.tx_flush();
        nrf.write_register(NRF_REGISTER::NRF_STATUS, nrf24l01::NRF_STATUS::MAX_RT); //clear previous RT bit
        nrf.write_register(NRF_REGISTER::NRF_STATUS, nrf24l01::NRF_STATUS::TX_DS); //clear previous sent bit
